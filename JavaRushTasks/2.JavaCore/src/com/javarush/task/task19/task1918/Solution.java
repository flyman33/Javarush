...
public class Solution {
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i <= text.length() - closeTag.length(); i++){    // Проходим по тексту
            if (text.substring(i).startsWith(openTag)) queue.addFirst(i);             // Записываем индекс открывающегося тега в очередь первым элементом
            if (text.substring(i).startsWith(closeTag)){                              // Если встретили закрывающися тег, то:
                ...
                if (queue.isEmpty()) while(!tempQueue.isEmpty()) System.out.println(tempQueue.removeLast()); // Если в очереди тегов закончилисm открывающиеся, начинаем выводить на печать из очереди-2
            }
        }
